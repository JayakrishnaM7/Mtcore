...
@Component
public class InvestmentType implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private InvestmentTypeRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(InvestmentType.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+InvestmentType.class.getName());
         }else{
