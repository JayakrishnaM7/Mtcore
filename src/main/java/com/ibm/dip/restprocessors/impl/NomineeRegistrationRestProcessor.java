...
@Service
public class NomineeRegistrationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(NomineeRegistrationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.nomineeregistration.hosturl}")
	private String url;
	
	@Value("${spring.api.nomineeregistration.context-path}")
	private String contextPath;

	@Value("${spring.api.nomineeregistration.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  NomineeRegistrationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ NomineeRegistrationRestProcessor.class.getName());
