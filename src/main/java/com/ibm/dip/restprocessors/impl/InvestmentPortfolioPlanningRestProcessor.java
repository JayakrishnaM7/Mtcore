...
@Service
public class InvestmentPortfolioPlanningRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(InvestmentPortfolioPlanningRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.investmentportfolioplanning.hosturl}")
	private String url;
	
	@Value("${spring.api.investmentportfolioplanning.context-path}")
	private String contextPath;

	@Value("${spring.api.investmentportfolioplanning.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  InvestmentPortfolioPlanningRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ InvestmentPortfolioPlanningRestProcessor.class.getName());
