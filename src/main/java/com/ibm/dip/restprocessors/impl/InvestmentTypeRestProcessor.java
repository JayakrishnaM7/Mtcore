...
@Service
public class InvestmentTypeRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(InvestmentTypeRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.investmenttype.hosturl}")
	private String url;
	
	@Value("${spring.api.investmenttype.context-path}")
	private String contextPath;

	@Value("${spring.api.investmenttype.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  InvestmentTypeRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ InvestmentTypeRestProcessor.class.getName());
