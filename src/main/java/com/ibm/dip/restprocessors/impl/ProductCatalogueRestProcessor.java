...
@Service
public class ProductCatalogueRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(ProductCatalogueRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.productcatalogue.hosturl}")
	private String url;
	
	@Value("${spring.api.productcatalogue.context-path}")
	private String contextPath;

	@Value("${spring.api.productcatalogue.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  ProductCatalogueRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ ProductCatalogueRestProcessor.class.getName());
