...
@Service
public class NotificationServiceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(NotificationServiceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.notificationservice.hosturl}")
	private String url;
	
	@Value("${spring.api.notificationservice.context-path}")
	private String contextPath;

	@Value("${spring.api.notificationservice.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  NotificationServiceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ NotificationServiceRestProcessor.class.getName());
